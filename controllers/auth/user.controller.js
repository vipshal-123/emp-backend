...
export const signupSendOtp = async (req, reply) => {
    try {
        const { body } = req
        const otpCount = body.otpCount || 0

        const findUser = await User.findOne({ where: { email: body.email }, attributes: ['id', 'isEmailVerified'], raw: true })
        ...
        let createUser = { id: findUser?.id, email: body.email }
        ...
        if (!isEmpty(findUser) && !findUser.isEmailVerified) {
            await Security.update(
                ...
                { where: { userId: findUser?.id, type: 'activation_mail' } },
            )
        }
...
export const verifySignupOtp = async (req, reply) => {
    try {
        ...
        const [updatedCount] = await User.update({ isEmailVerified: true }, { where: { id: decryptToken.id } })
...
export const resendOtp = async (req, reply) => {
    try {
        ...
        const user = await User.findOne({ where: { id: tokenId?.id }, attributes: ['id'] })
        ...
        const securityData = await Security.findOne({ where: { userId: user.id, type: tokenId?.type, mode: tokenId?.mode } })
...
export const createPassword = async (req, reply) => {
    try {
        const { body, cookies } = req
        ...
        const userData = await User.findByPk(decryptData.id)
        ...
        let findSecurity = await Security.findOne({ where: { userId: userData.id } })
        ...
        reply
            .setCookie('refreshToken', refreshToken, {
                maxAge: ms(config.REFRESH_TOKEN_EXPIRATION) / 1000,
                httpOnly: true,
                sameSite: 'none',
                secure: true,
                partitioned: true,
...
export const signin = async (req, reply) => {
    try {
        ...
        const userSecurity = await Security.findOne({ where: { userId: user.id }, raw: true })
...
export const refreshToken = async (req, reply) => {
    try {
        ...
        const storedToken = await Token.findOne({ where: { refreshToken: cookies?.refreshToken }, raw: true })
        ...
        if (new Date() > new Date(storedToken.expiresAt)) {
            await Token.destroy({ where: { refreshToken: cookies?.refreshToken } })
        ...
        let decoded = null
        try {
            decoded = jwt.verify(cookies?.refreshToken, config.JWT_SECRET)
